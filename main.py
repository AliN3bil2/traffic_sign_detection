...
class HybridSignSystem:
    ...
    def process_frame(self, frame):
        ...
        for box in detections.boxes:
            ...
            if final_conf < 0.75:
                yolo_name = self.class_names.get(yolo_cls, f"Unknown ({yolo_cls})")
                cls_name = self.class_names.get(cls_id, f"Unknown ({cls_id})")
                print(f"Ignored: YOLO={yolo_name} ({yolo_conf:.2f}) | Classifier={cls_name} ({cls_conf:.2f}) => Final={final_conf:.2f}")
                #print(f"Ignored: YOLO={yolo_cls}({yolo_conf:.2f}) | Classifier={cls_id}({cls_conf:.2f}) => Final={final_conf:.2f}")
                continue
